...
LONG _RunWithDll_RemotePrepareLoadEvent(__in HANDLE hProcess , __out HANDLE* phLocalLoadEvent , __out HANDLE* phRemoteLoadEvent)
{
	LONG			FinalStatus = -1;

	BOOL			bFlag = FALSE;
	
	HANDLE			hLocalLoadEvent = NULL;
	HANDLE			hRemoteLoadEvent = NULL;
	

	do 
	{
		hLocalLoadEvent = CreateEventW( NULL , TRUE , FALSE , NULL);
		if ( NULL == hLocalLoadEvent )
		{
			break;
		}
		
		bFlag = DuplicateHandle(
			GetCurrentProcess(),
			hLocalLoadEvent , 
			hProcess ,
			&hRemoteLoadEvent,
		...
		if ( !bFlag )
		{
			CloseHandle(hLocalLoadEvent);
			hLocalLoadEvent = NULL;
	...
	{
		*phLocalLoadEvent = hLocalLoadEvent;
	}

	if ( NULL != phRemoteLoadEvent)
	{
		*phRemoteLoadEvent = hRemoteLoadEvent;
	}
...
LONG _RunWithDll_BuildHookLdrLoadDllShellCode(
	__in HANDLE hTargetProcess ,
	__in BOOL bTarget64 , 
	__in void* pRemoteUnicodeString ,
	__in void* pRemoteLdrLoadDll ,
	__in void* pRemoteNtSetEvent ,
	__in HANDLE hRemoteLoadEvent ,
	__out void** ppCode , 
	__out ULONG* pulCodeSize
)
{	
	LONG		FinalStatus = -1;
	...
	unsigned char amd64_template[242] = {
		
	};

	unsigned char i386_template[117] = {
		0x90, 0x55, 0x8B, 0xEC, 0x83, 0xE4, 0xF0, 0x81, 0xEC, 0x00, 0x02, 0x00, 0x00, 0x9C, 0x60, 0x56,
		0x57, 0x51, 0xEB, 0x51, 0xBF, 0x11, 0x11, 0x11, 0x11, 0xB9, 0x05, 0x00, 0x00, 0x00, 0xF3, 0xA4,
		0x59, 0x5F, 0x5E, 0x8D, 0x45, 0xE0, 0xC7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x50, 0x68, 0x44, 0x44,
		0x44, 0x44, 0xB8, 0x55, 0x55, 0x55, 0x55, 0xFF, 0xD0, 0x8D, 0x45, 0xE0, 0xC7, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x50, 0x68, 0x22, 0x22, 0x22, 0x22, 0x6A, 0x00, 0xFF, 0x75, 0x08, 0xB8, 0x11, 0x11,
		0x11, 0x11, 0xFF, 0xD0, 0x61, 0x9D, 0x81, 0xC4, 0x00, 0x02, 0x00, 0x00, 0x8B, 0xE5, 0x5D, 0x68,
		0x11, 0x11, 0x11, 0x11, 0xC3, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x5E, 0x83, 0xC6, 0x06, 0xEB, 0xA4,
		0x33, 0x33, 0x33, 0x33, 0x90
	};
	...
	do 
	{
		...
		if (bTarget64)
		{
			...
			_RunWithDll_ReplaceULONG64(
				pShellcode,
				ulCodeSize,
				(ULONG64)0x4444444444444444,
				(ULONG64)hRemoteLoadEvent,
				TRUE				
			);
			...
			_RunWithDll_ReplaceULONG64(
				pShellcode,
				ulCodeSize,
				(ULONG64)0x5555555555555555,
				(ULONG64)pRemoteNtSetEvent,
				TRUE				
			);
		}
		else
		{
			...
			_RunWithDll_ReplaceULONG32(
				...
				ulCodeSize,
				(ULONG64)0x44444444,
				(ULONG)((ULONG64)hRemoteLoadEvent),
				TRUE				
			);
			...
			_RunWithDll_ReplaceULONG32(
				...
				ulCodeSize,
				(ULONG64)0x55555555,
				(ULONG)((ULONG64)pRemoteNtSetEvent),
				TRUE				
			);
		}
...
LONG _RunWithDll_LdrInject(__in HANDLE hTargetProcess , __in HANDLE hTargetThread ,  __in WCHAR* pszTargetDll )
{
	LONG FinalStatus = -1;
	LONG Status = 0;
	...
	do 
	{
		...
		Status = _RunWithDll_RemotePrepareLoadEvent(hTargetProcess , &hLocalLoadEvent , &hRemoteLoadEvent);
		if (0 != Status)
		{
			break;
		}
		...
		WaitForSingleObject(hLocalLoadEvent , INFINITE);
...
LONG RunWithDllW(__in WCHAR* pszApplication , __in_opt WCHAR* pszCommandline , __in WCHAR* pszTargetDll , __in ULONG nWaitTime)
{
	...
	do 
	{
		...
		nRet = _RunWithDll_LdrInject( stProcessInfo.hProcess , stProcessInfo.hThread , pszTargetDll );
		if ( 0 != nRet)
		{
			break;
		}
